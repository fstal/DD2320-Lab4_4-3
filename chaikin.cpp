 ...
 *  Assign done by : Filip Stål, Joel Weidenmark, Hannes Bennet, Alexander Nordh
                            ...
                            std::vector<vec3>& Curve)
{
    ...
    for(size_t i(0);i<ControlPolygon.size();i++)
    {
        ...
        const vec3 currentShit = glm::normalize(LeftPoint - prevPoint);
        const vec3 nextShit = glm::normalize(RightPoint - LeftPoint);
        //const vec3 prev = glm::normalize(PrePoint);
        
        float angle = glm::acos(glm::dot(currentShit, nextShit));
        printf( "%6.4lf", angle );
        printf( " - - - " );


        if(angle < 0.1 || angle > 3.04 ){
    ...
    kattmat = Curve;
    goToZero = MinNumDesiredPoints - 50;
    if(goToZero > 0){
        CornerCutting(kattmat, goToZero, Curve);
    }
